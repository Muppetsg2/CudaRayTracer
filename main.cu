...
#pragma region Allocations
    ...
    checkCudaErrors(cudaMallocManaged((void**)&d_scene, sizeof(SceneDescription)));

    d_scene->cam = h_scene->cam;

    d_scene->materialsCount = h_scene->materialsCount;
    size_t bytes = d_scene->materialsCount * sizeof(MaterialDesc);
    checkCudaErrors(cudaMallocManaged((void**)&d_scene->materials, bytes));
    checkCudaErrors(cudaMemcpy(d_scene->materials, h_scene->materials, bytes, cudaMemcpyHostToDevice));

    d_scene->objectsCount = h_scene->objectsCount;
    bytes = d_scene->objectsCount * sizeof(ObjectDesc);
    checkCudaErrors(cudaMallocManaged((void**)&d_scene->objects, bytes));
    checkCudaErrors(cudaMemcpy(d_scene->objects, h_scene->objects, bytes, cudaMemcpyHostToDevice));

    d_scene->lightsCount = h_scene->lightsCount;
    bytes = d_scene->lightsCount * sizeof(LightDesc);
    checkCudaErrors(cudaMallocManaged((void**)&d_scene->lights, bytes));
    checkCudaErrors(cudaMemcpy(d_scene->lights, h_scene->lights, bytes, cudaMemcpyHostToDevice));
